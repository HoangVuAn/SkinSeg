...
from Utils.losses import dice_loss, corr_loss_nor
...
def main(config):
    if args.wandb_id is not None:
        wandb.init(
            project="SkinSeg", 
            name=f"RankMatchv2_fold{config.fold}", 
    ...
    model = model.cuda()
    criterion = [nn.BCELoss(), dice_loss, corr_loss_nor]
...
def train_val(config, model, train_loader, val_loader, criterion, optimizer, scheduler, start_epoch, best_metrics, args):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (batch,
            ...
            loss = loss_x + ((loss_u_s1 * 0.15 + loss_u_s2 * 0.15 + loss_u_w_fp * 0.5) + 0.2 * (loss_c_s1 + loss_c_s2) / 2.0) * (sup_batch_len / unsup_batch_len) * consistency_weight
